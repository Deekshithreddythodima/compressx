...
class Codec {
    ...
    encode(data) {
        ...
        if (mp.size === 1) {
            ...
            let output_message = "Congratulations Compression Completed!!" + '\n' + "Compression Ratio : " + (data.length / final_string.length).toPrecision(2);
            return [final_string, output_message];
        }
    ...
    decode(data) {
        ...
        if (temp === "zer") {
            let decoded_data = "";
            let output_message = "Congratulations De-Compression Completed!!";
...
window.onload = function () {
    ...
    submitBtn.onclick = function () {
        ...
        if (extension != "txt") {
            alert("Invalid file type (." + extension + ") \nPlease upload a .txt file and try again");
            return;
        }
